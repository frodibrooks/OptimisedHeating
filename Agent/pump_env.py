...
class wds():
    def __init__(self,
        ...
        # Calculate peak heads and add debugging
        peak_heads = []
        for key in nomHCurvePolyDict.keys():
            try:
                max_q = np.max(nomHCurvePtsDict[key][:, 0])
                opti_result = minimize(-nomHCurvePolyDict[key], x0=1, bounds=[(0, max_q)])
                peak_value = nomHCurvePolyDict[key](opti_result.x[0])
                peak_heads.append(peak_value)
            except Exception as e:
                print(f"Error calculating peak head for curve {key}: {e}")

        # Check if peak_heads contains valid values before calculating maxHead
        if peak_heads:
            self.maxHead = np.max(peak_heads)
        else:
            print("Error: peak_heads is empty, cannot calculate maxHead")
